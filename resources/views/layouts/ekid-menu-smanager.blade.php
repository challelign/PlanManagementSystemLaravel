...
<div class="col-md-12 topnav container-fluid bg-info justify-content-center" id="myTopnav">
    <a class="{{'smanager/home' == request()->path()?'active':''}}"
       href="{{route('smanager')}}">ያልታዩ የሰራተኞች አቅድ
        <span class="badge badge-danger">{{$r}}</span></a>

    <a class=" {{'smanager/mazegaj-ekid' == request()->path()?'active':''}}"
       href="{{route('mazegaj-ekid')}}">ያልታዩ የምክትል አዘጋጅ(ም/ዳይሬክተር) አቅድ
        <span class="badge badge-danger">{{$h}}</span></a>

    <a class="{{'smanager/wanaazegaj-ekid' == request()->path()?'active':''}}"
       href="{{route('wanaazegaj-ekid')}}">ያልታዩ ዋና አዘጋጅ(ዋና ዳይሬክተር) አቅድ <span class="badge badge-danger">{{$sh}}</span>
    </a>
